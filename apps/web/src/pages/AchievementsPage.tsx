...
function toReadableBadgeName(value: string): string {
  return value
    .split(/[-_]+/g)
    .filter(Boolean)
    ...
    <section className="achievements-page">
      ...
      <section className="clues-summary" aria-label="Путь игрока">
        ...
        <article>
          ...
          <p className="section-meta">{progress.badges.slice(0, 3).map(toReadableBadgeName).join(', ') || 'Пока нет — завершите еженедельную миссию.'}</p>
        </article>
        <article>
